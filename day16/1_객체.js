...
객체[ Object ]
    ...
    - 객체지향 프로그래밍
        ...
        - 주체(나/개발자/실체) <--------------> 컴퓨터(컴퓨터/실체)
            즉) 내(개발자/주체)가 아닌 컴퓨터에서 실체물을 만들어내면 만들어진 것들은 모두 객체
        - 생활속 객체
            - 안경 , 의자 , 컴퓨터 , CPU , 강의실(공간) , 책 , 공기 , 종이
            - 사람(객체) : 기준(!!) 김현수(주체) ---> 박희만(객체)
                        - 김현수 기준으로는 김현수 주체가 되고 김현수가 바라보는 박희만은 객체가 된다.
            - 컴퓨터속 객체
                - 교유한 소겅 또는 행동 가진다.
                - 물리적으로 존재하거나 논리(추상)적으로 존재하는 것들
                - 인기가 많은 이유 : 세상의 모든 것들을 (성질/행동)을 디지털화 할 수 있다.
                - 객체지향 프로그래밍 언어 : JS , JAVA , PYTHON , C# 등등

    1. JS 객체 : 1. 클래스 기반의 (class-free object) 객체 2. 클래스가 없는 객체 (class-free object) 객체
