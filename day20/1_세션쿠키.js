/*
    객체 : 여러가지 자료를 하나의 자료 타입 구조
        객체 2개 :
            1. 내가만든객체
            2.(미리존재한)남이만든객체 : document , console , neww Date() 등등
                -> 남이 미리 데이터/함수 제공 함으로써 빠른 개발이 가능하다. ( 도구 제공 )
                -> 내가 집을 만드는데 도구는 사서 사용한다. ( 라이브러리 )

    세션 과 쿠키
        - 브라우저( HTTP 통신 , HTML/CSS/JS 코드를 렌더링/그려주는 프로그램 )
        - 브라우저 F12 -> [NETWORK] -> 왼쪽사이드메뉴 ( local storage(쿠키) / session storage(세션) )

        1. 세션 : 사용자가 웹페이지를 방문하고 있는 동안 메모리를 저장 하는 공간 , 브라우저 꺼지면 메모리 삭제 된다.
            ex) 검색엔진 , 임시데이터
            객체 : sessionStorage

        2. 쿠키 : 사용자가 웹페이지를 방문이 끝나도 메모리를 저장 하는 공간 , 브라우저 꺼져도 메모리 유지 된다. ( 사용자가 직접 삭제 )
            ex) 자동로그인 기능 구현
            객체 : localStoarge

        sessionStorage / localStoarge 객체의 속성/기능 제공 
            - .(도트/접근)연산자 : 객체내 속성 값 접근/호출 사용

        1. setItem( 'key' , value  ) : 세션 / 쿠키 값 저장 하는 함수 , key 와 value 한쌍으로 저장 구조
        2. getItem( 'key' ) : 세선 / 쿠키 값 호출 하는 함수 , key를 호출함으로써 value 가 반환되는 구조
    ...
    J(java)S(script)O(object)N(notation) 객체 : 자바스크립트 객체 문법의 형식
        - 실무에서 많이 사용되는 형식 ; JSON/XML/CSY
        1. JSON.stringfy( { 속성명 : 값 , 속성명 : 값 } ) : { } ---> 문자열 : '{ 속성명 : 값 , 속성명 : 값 }'  )
...
// [2] 세션 / 쿠키 데이터 삽입 , 속성명/변수명/키/필드명(데이터의 상징적인 이름 / 비슷하게 쓴다)
