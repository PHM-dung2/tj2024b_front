/*
    선언
        변수 let , 상수 const , 함수 function
    변수 유효범위
        1. 전역 변수 : 특정한 { } 안에서 선언 하지 않은 변수
        2. 지역 변수 : 특정한 { } 안에서 선언 한 변수
            { } : if(){} , for(){} , function(){}
              - 특징 : { } 사용중 일때만 메모리 할당. 사용이 끝나면 메모리 자동 초기화
        3. 매개 변수 : 함수호출시 인자값을 저장하는 변수
*/
//============================= 전역 / JS 전체 / 대한민국 ==================================
let 전역변수 = '대한민국'

//============================= 지역 / IF 구역 / 인천광역시 ================================
if( true ){
    let 지역변수1 = '인천광역시'
    console.log( 전역변수 )
    console.log( 지역변수1 )
    //========================= 지역 / FOR 구역 / 부평구 ===================================
    for( let i = 0 ; i < 1 ; i++ ){
        let 지역변수2 = '부평구'
        console.log(전역변수)   // 대한민국 ok
        console.log(지역변수1)  // 인천광역시 ok
        console.log(지역변수2)  // 부평구 ok
    }
    console.log(전역변수)     // 대한민국 ok
    console.log(지역변수1)      // 인천광역시 ok
    // console.log(지역변수2)      // 부평구 fail
} // } end
//============================= 지역 / 함수 구역 / 수원 =====================================
function 수원( ){
    let 지역변수3 = '수원시'
    console.log(전역변수)   // 대한민국 ok
    // console.log(지역변수1) // 인천광연시 fail
    // console.log(지역변수2) // 부평구 fail
    console.log(지역변수3) // 수원시 ok
}

// [1] 활용
let star = '' // 전역 변수
for( let i = 1 ; i<=10 ; i++){
    //i는 1부터 10까지 1씩 증가반복
    // let star = '' // 지연변수
    star += '*' // 누적합계
} //for end

console.log( star ); 
// for에서 선언된 변수는 for 안에서만 사용한다.

// [2] 활용2
let 이름배열 = [ ]
function 등록함수(){
    let 이름 = prompt('이름:') // 지역변수 , 이름을 입력받아 배열 저장
    이름배열.push( 이름 ); // 지역변수 값을 전역변수 배열에 추가한다.
} // for end

// = 입력 1번의 함수 종료되었을떄 변수몇개?? 데이터??
// - 이름(지역변수)X , 이름배열(전역변수)O , '유재석'(데이터) O
// - 이름배열 = { '유재석' }

// JS파일 사용 범위
// 1. <head> <script src='4_함수.js'></script> </head>
// body 내 마크업들이 구성 되기 전에 스크립트 읽어들인다.

// 2. <body> </body> 마크업 가장 하단에 <script src='4_함수.js'></script>

// 3. 여러 js파일 하나의 html 에서 호출할 때 변수/함수 공유
