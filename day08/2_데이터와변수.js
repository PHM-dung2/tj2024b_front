/*
    JS 의 데이터 기본 타입
        1. 문자열(String)   :   " " 또는 ' ' 또는 ` `(백틱-키보드TAB위) 으로 감싼 자료
            10(숫자타입) vs "10"(문자타입) : 자료는 10 같다. 자료의 타입은 다르다.
            예) '안녕하세요', '유재석' , `<div> </div>`
        2. 숫자(Number)
            에) 10 , 7, 3.14
        3. 불리언(boolean)
    ...
    변수
        - 변수(변하는 수) vs 상수(고정된 수)
            1. 하나의 자료/데이터( 참조주소값 ; 위치 저장 ) 를 저장할 수 있는 메모리 공간
            2. let 키워드 , const 키워드
...
// [1] 변수 선언 하는 방법
    ...
    // 2. 변수명 : 데이터/자료 를 저장하는 메모리의 상징적인 이름(식별/구분용-class/id같은 개념)
    //          - 중복 불가능 , 숫자로 싲가 , 띄어쓰기(컴퓨터; '-' , '_' 사용) 불가능
    // 3. = : 대입 , 오른쪽 항(값) 을 왼쪽 항(값) 대입하는 연산자
    // 4. 리터럴 / 값 : 변수에 대입할 리터럴/값 그 자체
let 변수상자1 = 10;

// [2] 변수(냉장고) 사용 하는 이유 : 값 저장 , 값 재사용 , 그 자체 값의 의미 부여
console.log( 10 );
console.log( 변수상자1 );
변수상자1 = 20;
변수상자1 = 20 + 10 ;   // 20  + 10 했을 때 새로운 리터럴 30 대입한다.

// [3] 변수의 값 변경 하기
    // 1. 변경할 변수명 = 새로운 값    : 변경할 변수명 = 새로운 값
변수상자1="유재석";
console.log( 변수상자1 )
...
const 고정상자 = '안녕하세요'
// [7] 상수 수정 불가능.
고정상자 = 10;  // Assignment to constant variable.(상수 변수에 할당)
